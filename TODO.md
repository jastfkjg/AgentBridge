...
- [x] 增加 Claude Desktop / Claude Code / Codex MCP 配置示例：`agentbridge mcp-config <kit>`
- [ ] 增加端到端示例：启动 mock HTTP API，连接 MCP client，完成一次真实读写操作

## Phase 2: 接入体验

- [ ] `agentbridge init --interactive`：交互式配置 OpenAPI 路径、base URL、认证方式、输出目录
- [x] `agentbridge init`：生成、校验并打印下一步命令
- [x] `agentbridge doctor`：检查 kit 完整性、执行模式配置和高风险工具策略
- [x] `agentbridge validate`：校验 `agentbridge-kit/v1` 协议、tools、guardrails、transport 和潜在 secret
- [x] 生成 MCP client 配置片段，减少用户手动接线
- [ ] `agentbridge doctor --probe`：探测目标 API 连通性、认证和只读工具样例调用
- [x] 生成边界：不修改目标项目，所有产物写入用户指定的新目录

## Phase 3: 执行安全与治理

- [x] per-tool allowlist / denylist
- [x] 全局只读模式和按风险级别禁用执行
- [x] 审计日志：记录 tool、args、user confirmation、planned call、HTTP response summary
- [x] 高风险确认文本：展示工具、风险、风险理由、计划调用和参数
- [x] 请求预览：展示 method、URL、headers 摘要、body、风险理由，并脱敏敏感 header
- [ ] 强确认文本：要求用户输入对象 ID 或操作摘要后才允许执行高风险操作
