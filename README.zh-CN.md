...
AgentBridge 在理解已有项目时依赖 AI 后端。确定性扫描器、正则和规则信号只作为 AI agent 的候选证据，而不是最终项目模型。只有 schema-only 的 OpenAPI 到 MCP 快速路径适合使用 `--no-ai`；目录级项目分析应使用 Claude Agent SDK 或其他 Anthropic 兼容提供商。
